...
function displayCharacter(characterData)
{
  ...
  if(!(characterData.id in characters))
  {
    ...
    {
      characterData.id = "narrator";
    }
    else if("mc" in characters)
    {
      characterData.id = "mc";
    }
    else
    {
      characterData.id = Object.keys(characters)[0];
    }
...
function displayChoices(choices)
{
  //choices holds and array of choice, though there is sometimes only one element in it.
    ...
    var choiceText = resolvePronounPlaceholders(choice.text);
    $("#choices").append("<div class='choice' data-target="+choice.target+">"+choiceText+"</a></div>");
	}
}

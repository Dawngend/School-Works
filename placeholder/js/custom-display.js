...
function onChoiceClicked(targetSceneId)
{
	...
	if(targetSceneId.toLowerCase() == "first_scene_start" && (currentSceneId == "game_end" || currentSceneId == "end_game" || currentSceneId == "scene_end"))
	{
		resetGameState("first_scene_start");
		return true;
	}

	if(targetSceneId.toLowerCase() == "restart_gender")
	{
		restartStory();
		return true;
	}

	if(targetSceneId.toLowerCase() == "main_menu")
	{
		resetAndReload();
		return true;
	}

"""Re-run the validator and build Pamesa_DawnAndrei_IT0123_M4_AI_REST_API.zip."""
import subprocess, sys, zipfile
from pathlib import Path

here = Path(__file__).parent
work = here / "it0123-m4-ai-rest-api"
...
print(out)
shots = sorted((work / "evidence").glob("*.png"))
if len(shots) < 8:
    print(f"WARNING: only {len(shots)} of 8 screenshots in evidence/")
zip_path = here / "Pamesa_DawnAndrei_IT0123_M4_AI_REST_API.zip"
with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as z:
    for f in sorted(work.rglob("*")):
        if f.is_file() and f.name != ".keep":
            z.write(f, Path(work.name) / f.relative_to(work))
    sheet = "Pamesa_DawnAndrei_IT0123_M4_ActivitySheet.docx"
